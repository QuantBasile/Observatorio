...
def make_fake_raptor_from_underlyings(underlyings: pd.DataFrame, n_rows: int = 1_000_000, seed: int = 123) -> pd.DataFrame:
    ...
    - includes columns used by actions: issuer/product/callput/Bid/Ask
    """
    ...
    product = rng.choice(["Producto1", "Producto2", "Producto3"], size=n_rows)
    callput = rng.choice(["Call", "Put"], size=n_rows)
    ...
    for c in ["scheme_id", "underlying_isin", "underlying_wkn", "issuer", "currency", "product", "callput"]:
        if c in df.columns:
