...
class DataTable(ttk.Frame):
    ...
    def __init__(self, parent: tk.Misc, on_copy=None):
        ...
        # Very light red highlight for rows where Issuer == HSBC
        self.tree.tag_configure("issuer_hsbc", background="#ffecec")
            ...
            if has_eventnext:
                try:
                    ...
                    if pd.notna(ev) and ev.date() == today:
                        tags.append("event_today")
                except Exception:
                    pass
            if has_issuer:
                try:
                    if str(row["Issuer"]).strip().upper() == "HSBC":
                        tags.append("issuer_hsbc")
                except Exception:
                    pass
